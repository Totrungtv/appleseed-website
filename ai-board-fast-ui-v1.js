/* APPLE SEED AI BOARD — FAST RESULT + TYPEWRITER V1 */
(()=>{
  'use strict';
  ...
  function typePre(pre,text){
    ...
    const tick=()=>{
      ...
      const box=result; if(box.scrollHeight>box.clientHeight)box.scrollTop=box.scrollHeight;
  ...
  function animate(){
    ...
    if(!text.trim()||pre.dataset.asTyped===text)return;
    pre.dataset.asTyped=text;
    typePre(pre,text);
  }
