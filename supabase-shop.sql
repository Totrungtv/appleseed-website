...
alter table public.inventory_items
  add column if not exists brand_id bigint references public.inventory_brands(id) on delete set null;
...
create table if not exists public.inventory_items (
  ...
  replacement_price numeric(14,2) not null default 0,
...
-- Giữ tương thích với database đã tạo từ phiên bản SQL cũ.
alter table public.inventory_items
  add column if not exists category text;

alter table public.inventory_items
  add column if not exists replacement_price numeric(14,2) not null default 0;

alter table public.inventory_items
  add column if not exists low_stock_threshold integer not null default 2;

-- Ngày nhận/giao máy cho phiếu sửa chữa.
...
create index if not exists idx_transactions_sale_order on public.store_transactions(sale_order_id);
...
drop policy if exists "authenticated sales all" on public.sale_orders;
create policy "authenticated sales all" on public.sale_orders
for all to authenticated using (true) with check (true);
...
begin
  ...
  if new.inventory_item_id is not null then
    ...
    if v_stock is null then
      raise exception 'Hàng trong kho không tồn tại.';
    end if;

    if v_stock < new.quantity then
      raise exception 'Không đủ tồn kho. Hiện còn %, cần bán %.', v_stock, new.quantity;
    end if;
...
drop trigger if exists trg_sale_orders_updated_at on public.sale_orders;
create trigger trg_sale_orders_updated_at
before update on public.sale_orders
for each row execute function public.set_updated_at();

drop trigger if exists trg_sale_inventory on public.sale_orders;
create trigger trg_sale_inventory
after insert or update or delete on public.sale_orders
for each row execute function public.sync_sale_inventory();
