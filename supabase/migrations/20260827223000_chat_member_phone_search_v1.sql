...
where (q.text_q='' or lower(coalesce(p.full_name,'')) like '%'||q.text_q||'%' or (length(q.phone_q)>=3 and regexp_replace(coalesce(u.raw_user_meta_data->>'phone',u.raw_user_meta_data->>'phone_number',u.phone,''),'[^0-9]','','g') like '%'||q.phone_q||'%'))
