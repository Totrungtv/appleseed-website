/* APPLE SEED SITE BUILDER LOGIN + THEME BRIDGE V25 */
(function(){
'use strict';
...
function protectSelection(){
  document.querySelectorAll('.as-theme-grid,.as-full-theme-grid').forEach(grid=>{
    if(grid.__appleSeedSelectionGuard)return;
    const observer=new MutationObserver(()=>{
      if(grid.__appleSeedSelectionGuardBusy)return;
      grid.__appleSeedSelectionGuardBusy=true;
      try{mark()}finally{grid.__appleSeedSelectionGuardBusy=false}
    });
    observer.observe(grid,{subtree:true,attributes:true,attributeFilter:['class']});
    grid.__appleSeedSelectionGuard=observer;
  });
}
function save(v,key,field){v=norm(v);if(!v)return;try{if(typeof draft!=='undefined'&&draft)draft[field]=v;localStorage.setItem(key,v);if(typeof saveDraft==='function')saveDraft()}catch(_){}apply();mark();protectSelection()}
...
function installTheme(){window.addEventListener('click',themeIntercept,true);const f=document.getElementById('preview');f?.addEventListener('load',()=>{apply();mark();protectSelection()});try{if(typeof window.applyAll==='function'&&!window.applyAll.__appleSeedThemeWrapped){const original=window.applyAll;const wrapped=function(){const r=original.apply(this,arguments);try{mirrorPublishedToStorage();apply();mark();protectSelection()}catch(_){}return r};wrapped.__appleSeedThemeWrapped=true;window.applyAll=wrapped}}catch(_){}mark();protectSelection()}
