/* APPLE_SEED_THEME_SELECTION_BRIDGE_V16 */
(function(){
  'use strict';
  ...
  function setRootClass(r,p,v){
    if(!r)return;
    const wanted=p+v;
    if(r.classList.contains(wanted))return;
    for(let i=1;i<=30;i++)r.classList.remove(p+String(i).padStart(2,'0'));
    r.classList.add(wanted);
  }
  function link(d,id,href){
    let e=d.getElementById(id);
    if(!e){e=d.createElement('link');e.id=id;e.rel='stylesheet';e.href=href;(d.head||d.documentElement).appendChild(e)}
  }
  function applyPreview(){
    const f=document.getElementById('preview'),d=f&&f.contentDocument;
    if(!d||!d.documentElement)return;
    const c=getColor(),t=getFull();
    setRootClass(d.documentElement,'as-theme-',c);
    if(d.body)setRootClass(d.body,'as-theme-',c);
    setRootClass(d.documentElement,'as-full-theme-',t);
    if(d.body)setRootClass(d.body,'as-full-theme-',t);
    d.documentElement.classList.add('as-theme-scope','as-full-theme-scope');
    if(d.body)d.body.classList.add('as-theme-scope','as-full-theme-scope');
  ...
  function mark(){
    ...
    if(a)a.textContent='Đang chọn: Theme '+c+' · áp dụng cho toàn bộ WEB khi Xuất bản.';
    const b=document.getElementById('appleSeedFullThemeStatus');
    if(b)b.textContent='Đang chọn: Giao diện '+t+' · áp dụng cho toàn bộ WEB khi Xuất bản.';
  }
  function persistLocked(){
    try{
      if(lockedColor){localStorage.setItem(COLOR_KEY,lockedColor);if(typeof draft!=='undefined'&&draft)draft.theme_id=lockedColor}
      if(lockedFull){localStorage.setItem(FULL_KEY,lockedFull);if(typeof draft!=='undefined'&&draft)draft.full_theme_id=lockedFull}
    }catch(_){ }
  }
  function save(v,key){
    v=norm(v);if(!v)return;
    if(key===COLOR_KEY)lockedColor=v;else lockedFull=v;
    persistLocked();
    try{if(typeof saveDraft==='function')saveDraft()}catch(_){ }
    applyPreview();mark();
    setTimeout(enforce,0);setTimeout(enforce,100);setTimeout(enforce,350);
  }
  function enforce(){persistLocked();applyPreview();mark()}
  function interceptTheme(e){
    const el=e.target&&e.target.closest&&e.target.closest('.as-theme-choice,.as-full-theme-choice');
    if(!el)return;
    ...
    save(v,full?FULL_KEY:COLOR_KEY);
  }

  function loginBox(){return document.querySelector('.login')}
  function loginError(message){
    const box=loginBox();if(!box)return;
    const el=document.getElementById('loginMsg')||box.querySelector('.danger');
    if(el){el.textContent=message||'';el.style.display='block'}
  }
  function loginFields(){
    const box=loginBox();if(!box)return null;
    const email=box.querySelector('#email,input[type="email"],input[name="email"],#loginEmail');
    const password=box.querySelector('#password,input[type="password"],input[name="password"],#loginPassword');
    const button=box.querySelector('#loginBtn,button[type="submit"],button');
    if(!email||!password||!button)return null;
    return{box,email,password,button};
  }
  function withTimeout(p,ms,message){
    return Promise.race([p,new Promise((_,reject)=>setTimeout(()=>reject(new Error(message)),ms))]);
  }
  ...
  async function loginFallback(e){
    ...
    if(!client?.auth||typeof client.auth.signInWithPassword!=='function'){
      loginError('Không tải được hệ thống đăng nhập. Hãy tải lại trang.');return;
    }
    loginBusy=true;f.button.disabled=true;
    const oldText=f.button.textContent;f.button.textContent='Đang đăng nhập…';loginError('Đang xác thực tài khoản…');
    try{
      ...
      console.error('Apple Seed Builder login V16:',err);
  ...
  async function recoverExistingSession(){
    ...
    try{
      ...
      if(s.data?.session?.user){
        try{
          await verifyBuilderRole(s.data.session.user);
          box.style.display='none';
          const st=document.getElementById('status');if(st)st.textContent='Đã xác thực · Admin/Staff';
        }catch(e){console.warn('Builder existing session role:',e)}
      }
    }catch(_){ }
  }
  ...
  function install(){
    ...
    if(frame)frame.addEventListener('load',()=>{enforce();setTimeout(enforce,100);setTimeout(enforce,500)});
    window.addEventListener('pointerdown',interceptTheme,true);
    window.addEventListener('click',interceptTheme,true);
    ...
    enforceTimer=setInterval(enforce,1200);
    setTimeout(recoverExistingSession,0);
  }
