/* APPLE SEED SITE BUILDER LOGIN + THEME BRIDGE V22 */
(function(){
'use strict';
...
/* THEME: one authoritative state. draft wins; localStorage is fallback only. */
...
function storageValue(key){try{return norm(localStorage.getItem(key)||'')}catch(_){return''}}
function colorValue(){return draftValue('theme_id')||storageValue(COLOR_KEY)||'01'}
function fullValue(){return draftValue('full_theme_id')||storageValue(FULL_KEY)||'01'}
...
function installTheme(){window.addEventListener('click',themeIntercept,true);const f=document.getElementById('preview');f?.addEventListener('load',()=>{apply();mark()});
  /* loadPublished() calls applyAll() after its async remote snapshot is loaded. Wrap that single function so the remote draft becomes authoritative immediately. */
  try{if(typeof window.applyAll==='function'&&!window.applyAll.__appleSeedThemeWrapped){const original=window.applyAll;const wrapped=function(){const r=original.apply(this,arguments);try{apply();mark()}catch(_){}return r};wrapped.__appleSeedThemeWrapped=true;window.applyAll=wrapped}}catch(_){ }
  mark();
}
