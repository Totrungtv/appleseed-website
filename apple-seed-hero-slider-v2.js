/* Apple Seed Hero Slider V6 — single visual owner + clean first paint. */
(function(){
  'use strict';
  ...
  function css(){
    if(document.getElementById('apple-seed-slider-v6-css'))return;
    var s=document.createElement('style');s.id='apple-seed-slider-v6-css';
    s.textContent=''
      ...
      +'.apple-seed-hero-slide:after{content:"";position:absolute;inset:0;background:linear-gradient(90deg,rgba(0,0,0,.08),transparent 62%,rgba(0,0,0,.05));pointer-events:none;}'
      +'.apple-seed-hero-caption{position:absolute;left:7%;bottom:9%;max-width:min(620px,58%);padding:16px 22px;border-left:3px solid #d5a958;border-radius:0 16px 16px 0;background:linear-gradient(90deg,rgba(0,0,0,.62),rgba(0,0,0,.03));color:#fff;text-shadow:0 2px 14px rgba(0,0,0,.8);z-index:2;}'
      +'.apple-seed-hero-caption .k{font-size:10px;letter-spacing:3px;font-weight:900;opacity:.88}.apple-seed-hero-caption h2{font-size:clamp(24px,3.2vw,50px);line-height:1.02;margin:6px 0;font-weight:950;letter-spacing:-1.2px}.apple-seed-hero-caption p{font-size:12px;line-height:1.55;margin:0;opacity:.92;}'
      ...
      +'@media(max-width:760px){.as3-stage.apple-seed-slider-active{position:relative!important;width:calc(100vw - 48px)!important;max-width:calc(100vw - 48px)!important;min-width:0!important;height:auto!important;min-height:0!important;aspect-ratio:16/9!important;margin:18px auto 24px!important;padding:0!important;transform:none!important;}.apple-seed-hero-slider{border-radius:16px!important}.apple-seed-hero-caption{left:5%;bottom:9%;max-width:78%;padding:10px 13px}.apple-seed-hero-caption .k{font-size:7px;letter-spacing:2px}.apple-seed-hero-caption h2{font-size:25px}.apple-seed-hero-caption p{font-size:9px}.apple-seed-hero-arrow{width:36px;height:36px;font-size:29px}.apple-seed-hero-arrow.prev{left:8px}.apple-seed-hero-arrow.next{right:8px}.apple-seed-hero-dots{bottom:8px;gap:6px;padding:4px 7px}.apple-seed-hero-dot{width:6px;height:6px}.apple-seed-hero-dot.active{width:18px}}'
  ...
  function build(stage,c){
    ...
    c.images.forEach(function(raw,i){
      ...
      var img=document.createElement('img');img.src=sl.src;img.alt=sl.title||('Apple Seed banner '+(i+1));img.loading=i===0?'eager':'lazy';img.decoding='async';img.style.objectFit=sl.fit||'cover';img.style.objectPosition=sl.pos||'center';if(sl.fit==='contain')img.style.padding='4%';
      ...
      if(sl.title||sl.sub||sl.desc){var cap=document.createElement('div');cap.className='apple-seed-hero-caption';cap.innerHTML='<div class="k">'+esc(sl.sub||'APPLE SEED')+'</div><h2>'+esc(sl.title||'APPLE SEED')+'</h2><p>'+esc(sl.desc||'')+'</p>';slide.appendChild(cap)}
